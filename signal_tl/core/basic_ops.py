from signal_tl.core.base import Expression
from signal_tl.core.temporal_ops import *
...
class LogicOp(Expression):
    ...
    def _latex(self, printer=None):
        ...
        if self.nargs is None:
            args = [printer.doprint(i) for i in self.args]
            tex_args = tuple(r'\left( {} \right)'.format(arg) for arg in args)
            return ' {} '.format(symbol).join(tex_args)
        if self.nargs == 1:
            a = printer.doprint(self.args[0])
            return r'{} \left( {} \right)'.format(symbol, a)
        if self.nargs == 2:
            a, b = [printer.doprint(i) for i in self.args]
            return r'\left( {} \right) {} \left( {} \right)'.format(a, symbol, b)
