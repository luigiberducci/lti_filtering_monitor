...
def lti_filter_monitor(phi: stl.Expression, w: pd.DataFrame, time_stamps: list or tuple or np.ndarray = None,
                       dt: float = -1.0, window: str or tuple or float = 'boxcar'):
    """
    ...
    :type w: pd.DataFrame
    :param time_stamps: List of time points to get the robustness from (default: all points in `w`)
    :type time_stamps: list or tuple or np.ndarray
